...
def adjust_lr_exp(optimizer, base_lr, ep, total_ep, start_decay_at_ep):
    """
    Decay exponentially in the later phase of training.
    All parameters in the optimzier share the same learning rate.
    Args:
        optimzier: a pytorch Optimizer object
        base_lr: starting learning rate
        ep: current epoch, ep >= 1
        total_ep: total number of epochs to train
        start_decay_at_ep: start decaying at the Begining of this epoch
    Example:
        base_lr = 2e-4
        total_ep = 300
