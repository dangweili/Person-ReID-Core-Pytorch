...
__all__ = ['HACNN', 'HACNNExtractFeature']
...
class HACNN(nn.Module):
    ...
    """
    def __init__(self, num_classes, nchannels=[128, 256, 384], feat_dim=512, learn_region=True, use_gpu=True, **kwargs):
    ...
    def transform_theta(self, theta_i, region_idx):
        ...
        theta = torch.autograd.Variable((torch.zeros(theta_i.size(0), 2, 3).cuda()))
        for i in range(theta_i.size(0)):
            theta[i,:,:2] = scale_factors
        theta[:,:,-1] = theta_i
        # if self.use_gpu: theta = torch.autograd.Variable(theta.cuda())
        return theta
    ...
    def forward(self, x):
        ...
        if self.learn_region:
            ...
            for region_idx in range(4):
                ...
                x1_trans_i = F.upsample(x1_trans_i, (24, 28), mode='bilinear')
        ...
        if self.learn_region:
            ...
            for region_idx in range(4):
                ...
                x2_trans_i = F.upsample(x2_trans_i, (12, 14), mode='bilinear')
        ...
        if self.learn_region:
            ...
            for region_idx in range(4):
                ...
                x3_trans_i = F.upsample(x3_trans_i, (6, 7), mode='bilinear')
...
class HACNNExtractFeature(object):
