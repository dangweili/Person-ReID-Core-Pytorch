...
from core.model.PCBModel import PCBModel
from core.model.PCBModel import PCBExtractFeature 
...
class Config(object):
    def __init__(self):
        ...
        parser.add_argument('--resize', type=eval, default=(384, 128))
...
# for evaluation
feat_func = PCBExtractFeature(model_w)
# test only
if cfg.test_only:
...
for epoch in range(start_epoch, cfg.total_epochs):
    ...
    for step, (imgs, targets) in enumerate(train_loader):
        ...
        _, logits_list = model_w(imgs_var)
        loss = torch.sum(
        torch.cat([criterion(logits, targets_var) for logits in logits_list]))
